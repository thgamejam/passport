...
func (r *passportRepo) VerifyAccountTokenId(ctx context.Context) (accountId uint32, err error) {
	...
	// 解密Token
	loginToken, success := jwt.ValidateLoginToken(token, []byte(r.data.conf.VerifyEmailKey))
	if !success {
		// 错误 Token解析错误
...
func (r *passportRepo) SignLoginToken(ctx context.Context, accountID uint32) (token string, err error) {
	...
	sid, err := r.data.accountClient.CreateSession(ctx, &accountV1.CreateSessionReq{
		Id:        accountID,
		Ip:        ip,
		ExpiresAt: r.data.conf.LoginExpireTime,
	})
	if err != nil {
		return "", err
	}
	t, err := jwt.CreateLoginToken(jwt.LoginToken{
		...
		RenewalAt: time.Now().Add(r.data.conf.RenewalTime.AsDuration()).Unix(),
	}, []byte(r.data.conf.VerifyEmailKey), r.data.conf.RenewalTime)
	if err != nil {
		return "", err
	}
	return t, nil
